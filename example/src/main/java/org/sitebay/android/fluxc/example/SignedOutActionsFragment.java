...
import org.sitebay.android.fluxc.annotations.action.ActionBuilder;
import org.sitebay.android.fluxc.example.ThreeEditTextDialog.Listener;
...
import org.sitebay.android.fluxc.store.AccountStore.OnAuthEmailSent;
import org.sitebay.android.fluxc.store.AccountStore.OnAuthOptionsFetched;
import org.sitebay.android.fluxc.store.AccountStore.OnNewUserCreated;
...
import org.sitebay.android.fluxc.store.SiteStore.OnConnectSiteInfoChecked;
import org.sitebay.android.fluxc.store.SiteStore.OnSuggestedDomains;
import org.sitebay.android.fluxc.store.SiteStore.OnURLChecked;
import org.sitebay.android.fluxc.store.SiteStore.OnWPComSiteFetched;
...
public class SignedOutActionsFragment extends Fragment {
    ...
    private void showFetchWPComSiteDialog() {
        ...
        final EditText editText = new EditText(getActivity());
        editText.setSingleLine();
        alert.setMessage("Fetch the SiteBay.org site with URL:");
        alert.setView(editText);
        alert.setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int whichButton) {
                String url = editText.getText().toString();
    ...
    public void onUrlChecked(OnURLChecked event) {
        if (event.isError()) {
            ...
            prependToLog("OnUrlChecked: " + event.url + (event.isWPCom ? " is" : " is not")
                         + " accessible via SiteBay.org API");
        }
    }
