...
import org.sitebay.android.fluxc.store.AccountStore.IsAvailableErrorType;
import org.sitebay.android.fluxc.store.AccountStore.OnAvailabilityChecked;
...
public class ReleaseStack_AccountAvailabilityTest extends ReleaseStack_Base {
    ...
    public void testIsAvailableBlog() throws InterruptedException {
        ...
        mDispatcher.dispatch(AccountActionBuilder.newIsAvailableBlogAction("sitebay"));

        assertTrue(mCountDownLatch.await(TestUtils.DEFAULT_TIMEOUT_MS, TimeUnit.MILLISECONDS));

        assertEquals("sitebay", mLastEvent.value);
    ...
    public void testIsAvailableEmail() throws InterruptedException {
        ...
        mCountDownLatch = new CountDownLatch(1);
        mDispatcher.dispatch(AccountActionBuilder.newIsAvailableEmailAction("mobile@sitebay.org"));

        assertTrue(mCountDownLatch.await(TestUtils.DEFAULT_TIMEOUT_MS, TimeUnit.MILLISECONDS));

        assertEquals("mobile@sitebay.org", mLastEvent.value);
        assertFalse(mLastEvent.isAvailable);
