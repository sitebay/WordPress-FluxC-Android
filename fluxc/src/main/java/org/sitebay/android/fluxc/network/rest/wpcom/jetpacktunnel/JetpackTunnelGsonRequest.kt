...
/**
 * A request making a WP-API call to a Jetpack site via the SiteBay.org /jetpack-blogs/$site/rest-api/ tunnel.
 *
 * # Requests
 *
 ...
 * query={"status":"draft"}
 *
 * The path parameter is sent HTML-encoded so that it's discernible from the other arguments by SiteBay.org.
 * In this example, this would become a GET request to {JSON endpoint root}/wp/v2/posts/?status=draft.
 *
 * Any additional top-level params are received by the SiteBay.org API, and are not sent through to the
 ...
 * {"title":"A title"}
 *
 * Any additional top-level arguments are received by the SiteBay.org API, and are not sent through to the
 * WP-API endpoint.
 *
 * ## PUT/PATCH
 *
 ...
 * # Responses
 *
 * The SiteBay.org endpoint will return the response it received from the WP-API endpoint, wrapped in a `data`
...
object JetpackTunnelGsonRequest {
     ...
     * @param siteId the SiteBay.org site ID
